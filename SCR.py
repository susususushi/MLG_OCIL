# -*- coding: utf-8 -*-
"""
Created on Tue Mar  8 20:17:29 2022
    ...
    for run_time in range(args.run_time):
        ...
        for task_id, (x_train, y_train, labels) in enumerate(data_continuum):
            ...
            for batch_id, batch_data in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                combined_x_feat = model(combined_batch_x)#forward自带归一化
                #SCR part
                aug_batch_x = aug_transform(combined_batch_x)
                aug_x_feat = model(aug_batch_x)#forward自带归一化
                #shape = batch_x.size(0)+mem_x.size(0), 2 , feature_sizes, 2代表着对比学习
                ...
                if mem_x.size(0) > 0:
                    ...
                    for ep in range(1):
                        for i, batch_data in enumerate(rv_loader):
                            ...
                            x_feat = model(batch_x)#forward自带归一化
                            aug_x_feat = model(aug_batch_x)#forward自带归一化
