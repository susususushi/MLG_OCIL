        ...
        for i in range(n):
            ...
            with torch.no_grad():
                ...
                if self.accumulate_cnt[classes[i]].item() == 0:
                    # not appear
                    self.acc_mean_feat = torch.cat([self.acc_mean_feat, current_feat.unsqueeze(0)],
                                                   dim=0)  # accumulate mean feature
                    ...
                    self.mean_feat = torch.cat([self.mean_feat, norm_feat.unsqueeze(0)],
                                               dim=0)  # normalized accumulate mean feature
                    ...
                    self.acc_mean_feat[indice] = current_feat * (tot / now_tot) + self.acc_mean_feat[indice] * (
